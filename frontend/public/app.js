...
function displayScanHistory(history) {
    ...
    container.innerHTML = history.map(scan => `
       <div class="history-item" onclick="viewScanDetails(${scan.scan_id})">
           <div class="history-header">
               <span class="history-id">Scan #${scan.scan_id}</span>
               <span class="history-date">${formatTimestamp(scan.scan_date)}</span>
           </div>
...
function viewScanDetails(scanId) {
    ...
    // Show loading state
    document.getElementById('modalScanInfo').innerHTML = '<div class="modal-loading">Loading scan details...</div>';
    document.getElementById('modalStatsGrid').innerHTML = '';
    document.getElementById('modalResultsBody').innerHTML = '';

    // Request scan details from backend
    socket.emit('get_scan_details', { scan_id: scanId });
}

// Function to close scan details modal
...
// Function to filter modal results
function filterModalResults(filter) {
    if (filter) {
        currentModalFilter = filter;
        // Update active button
        document.querySelectorAll('#scanDetailsModal .filter-btn').forEach(btn => {
            btn.classList.toggle('active', btn.dataset.filter === filter);
        });
    }

    const searchTerm = document.getElementById('modalSearchInput').value.toLowerCase();
    const sortBy = document.getElementById('modalSortSelect').value;

    // Filter results
    let filteredResults = Object.entries(modalScanResults).filter(([ip, data]) => {
        // Filter by view mode
        if (currentModalFilter === 'online' && data.status !== 'online') return false;
        if (currentModalFilter === 'offline' && data.status !== 'offline') return false;
    ...
    // Update filter button counts
    const total = Object.keys(modalScanResults).length;
    const online = Object.values(modalScanResults).filter(host => host.status === 'online').length;
    const offline = total - online;

    document.querySelector('#scanDetailsModal [data-filter="all"]').textContent = `All (${total})`;
    document.querySelector('#scanDetailsModal [data-filter="online"]').textContent = `Online (${online})`;
    document.querySelector('#scanDetailsModal [data-filter="offline"]').textContent = `Offline (${offline})`;

    // Render table
    renderModalResultsTable(filteredResults);
}

// Function to render modal results table
function renderModalResultsTable(results) {
...
// Add this socket event handler in the initializeSocket function after the existing handlers
socket.on('scan_details', (data) => {
    ...
    modalScanResults = data.results;

    // Update scan info
    const scanInfo = data.scan_info;
    document.getElementById('modalScanInfo').innerHTML = `
        <div><strong>Scan Date:</strong> ${formatTimestamp(scanInfo.scan_date)}</div>
        <div><strong>Network Range:</strong> ${scanInfo.network_range || 'Auto-detected'}</div>
        <div><strong>Total Hosts:</strong> ${scanInfo.total_hosts}</div>
        <div><strong>Online Hosts:</strong> ${scanInfo.online_hosts}</div>
        <div><strong>Scan Duration:</strong> ${scanInfo.scan_duration?.toFixed(2)}s</div>
        ${scanInfo.notes ? `<div><strong>Notes:</strong> ${scanInfo.notes}</div>` : ''}
    `;

    // Update stats grid
    const hostsWithPorts = Object.values(data.results).filter(host => host.ports && host.ports.length > 0).length;
    document.getElementById('modalStatsGrid').innerHTML = `
        <div class="stat-card stat-blue">
            <div class="stat-number">${scanInfo.total_hosts}</div>
            <div class="stat-label">Total Hosts</div>
        </div>
        <div class="stat-card stat-green">
            <div class="stat-number">${scanInfo.online_hosts}</div>
            <div class="stat-label">Online Hosts</div>
        </div>
        <div class="stat-card stat-yellow">
            <div class="stat-number">${hostsWithPorts}</div>
            <div class="stat-label">With Open Ports</div>
        </div>
    `;

    // Reset filters and display results
    currentModalFilter = 'all';
    document.getElementById('modalSearchInput').value = '';
    document.getElementById('modalSortSelect').value = 'ip';
    filterModalResults();
});
...
window.filterModalResults = filterModalResults;